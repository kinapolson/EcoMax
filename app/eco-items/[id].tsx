...
export default function BusinessDetails() {
  ...
  useEffect(() => {
    fetch(`http://gavialoid-forensic-regan.ngrok-free.dev/get_b_items.php?item_id=${id}`)
      .then(res => res.json())
      .then(data => {
        if (data.status === "success") {
          setItem(data.item);
          fetch(`http://gavialoid-forensic-regan.ngrok-free.dev/get_b_items.php?b_id=${data.item.b_id}`)
            .then(res => res.json())
            .then(data2 => {
          ...
          <View style={styles.itemsGrid}>
            {similarItems.map((business) => (
              ...
              >
                ...
                <Image
                  source={{ uri: `http://gavialoid-forensic-regan.ngrok-free.dev/items/${business.image}` }}
